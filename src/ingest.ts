...
export async function ingestInbound(collective: Collective, parsed: ParsedMail, resendEmailId?: string) {
  ...
  if (!isAutoSubmitted(parsed)) {
    ...
    await notifyInbound(collective, (await getThread(thread.id))!, message).catch((err) => console.error('[notify] failed:', err))
  }
