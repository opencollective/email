...
test('no usable Stripe key: subscribe UI hidden, opening the inbox leads', async () => {
  const { run } = await import('../src/db.js')
  const { createSession } = await import('../src/auth.js')
  ...
  assert.match(html, /Open the inbox/, 'the free month leads when there is nothing to pay with')
